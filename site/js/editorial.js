...
let ticking=false;
function update(){ticking=false;if(story&&stage){const r=story.getBoundingClientRect();const p=Math.max(0,Math.min(1,-r.top/Math.max(1,r.height-innerHeight)));const spread=motion.matches?0:Math.sin(p*Math.PI);stage.style.setProperty('--spread',spread.toFixed(4));story.style.setProperty('--progress',p.toFixed(4));}const bar=document.querySelector('.mobile-contact'),lead=document.getElementById('lead');if(bar&&lead){const r=lead.getBoundingClientRect();bar.hidden=r.top<innerHeight&&r.bottom>0;}}
